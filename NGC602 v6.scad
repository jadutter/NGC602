...
Intensity2=Intensity*4-4;
	translate([loc[0],loc[1],0]){
		scale([1,1,1]){
			color("Purple")linear_extrude(height=Intensity2)circle(Standard*.8,$fn=fn);
		}
	}
}
...
module drawing(i){
	if(i==1){
		scale(scaling/.95){
			translate([0,move,base_boundaries[2]/2]){
				scale(inkscale){
					color("Blue")linear_extrude(file = "v4_NGC_602.dxf", height = Emboss, layer = "Filaments");
				}
			}
		}
	}
	if(i==2||i==3||i==4){
		scale(scaling/.95){
			translate([0,move,base_boundaries[2]/2]){
				scale(inkscale){
					color("Red")linear_extrude(file = "v4_NGC_602.dxf", height = Emboss, layer = "Gasses");
				}
			}
		}
		scale(scaling/.95){
...
module drawing2(i){
	...
	if(i==2||i==3||i==4){
		...
		scale(scaling/.95){
			translate([0,move,base_boundaries[2]/2]){
				scale(inkscale){
					color("White")linear_extrude(file = "v4_NGC_602.dxf", height = Emboss, layer = "Dust_t");
				}
			}
		}
		scale(scaling/.95){
			translate([0,17.687,base_boundaries[2]/2]){
				scale(inkscale){
					color("Blue")linear_extrude(file = "v4_NGC_602.dxf", height = Emboss+1, layer = "Gas_Dust_t");
				}
			}
		}
	}
...
module plate3(){
	...
	translate([transX,transY,0]){
		scale([scaling,scaling,1]){
			for(i=[0:49]){
				if(star[i][2]==3){
					star(Standard,log(star[i][3])+.75,star[i]);
				}
			}
		}
	drawing3(3);
...
module bits(){
//	translate([-4,-4,0]){
		translate([transX,transY,15]){
			scale([scaling,scaling,1]){
				for(i=[0:69]){
					translate([0,0,0])color("silver")object(Standard,gasses[i][0],gasses[i][1],gasses[i][3]/30+3);
...
//	}
	translate([transX,transY,0]){
		scale([scaling,scaling,1]){
			for(i=[0:29]){
				translate([0,0,20])color("Maroon")object(Standard,gas_dust[i][0],gas_dust[i][1],gas_dust[i][3]/30+3);
	...
	translate([transX,transY,0]){
		scale([scaling,scaling,1]){
			for(i=[0:62]){
				translate([0,0,40])color("Lime")object(Standard,dust[i][0],dust[i][1],dust[i][3]/3);
